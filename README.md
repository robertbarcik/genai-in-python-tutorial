...
Default model in every notebook: `gpt-5.6-luna`, the small, cheap model of the current generation (about 0.20 USD per million input tokens, 1.20 USD per million output). Running a whole notebook costs cents.

## Modules

The advanced modules (3 to 10) go from the simplest concepts to the toughest: app plumbing first, function calling second to last, open-source models last.
...
| 3 | [Conversations, Caching, Batch, Moderation](3_conversations_caching_batch/3_conversations_caching_batch.ipynb) | Keeping conversation state three ways, prompt caching, the Batch API and flex tier, the moderation endpoint |
| 4 | [Basics of Prompt Engineering](4_basics_of_prompt_engineering/4_basics_of_prompt_engineering.ipynb) | Reasoning effort and verbosity, clear instructions with reasons, roles and structure, few-shot, reducing hallucination, chaining calls |
| 5 | [Structured Outputs](5_structured_outputs/5_structured_outputs.ipynb) | Pydantic + `responses.parse`, what is sent under the hood, enums, optional fields, nested lists, refusals |
| 6 | [Retrieval-Augmented Generation (RAG)](6_retrieval_augmented_generation_RAG/6_retrieval_augmented_generation_RAG.ipynb) | Embed, store, search, generate, built by hand with pictures of each step; top-k; grounded answers |
| 7 | [Fine-Tuning](7_fine-tuning/7_fine-tuning.ipynb) | What LoRA changes inside a model (shown with numpy), behaviour vs knowledge, what it costs on OpenAI, hosted and on your own GPU, and why you will rarely need it. CPU only, no API key |
| 8 | [Built-in Tools](8_tools/8_tools.ipynb) | Web search, code interpreter, file search (hosted RAG), image generation, remote MCP, combining tools |
| 9 | [Function Calling](9_function_calling/9_function_calling.ipynb) | The two-sided flow, the loop that runs your functions and hands results back, `tool_choice` |
| 10 | [Hugging Face](10_hugging_face/10_hugging_face.ipynb) | Running open-source models locally with `transformers`. **Needs a GPU, Colab T4** |
...
Module 10 (Hugging Face) needs a GPU runtime: in Colab go to **Runtime > Change runtime type > T4 GPU** before running it.
...
Module 10 (Hugging Face) needs a GPU and installs `torch`/`transformers` itself in its first cell, so those packages are deliberately left out of `requirements.txt`; run it on Colab's free T4 GPU rather than locally.
